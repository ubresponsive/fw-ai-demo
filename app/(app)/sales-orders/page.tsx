...
function statusStyle(status: string) {
  switch (status) {
    ...
    case 'Picking': return 'bg-violet-50 text-violet-700 border-violet-200'
    case 'Picked (Loc...)': return 'bg-emerald-50 text-emerald-700 border-emerald-200'
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] flex-col bg-gray-50">
      ...
      <div className="flex items-center gap-1.5 px-4 py-2 text-sm text-gray-500 border-b border-gray-200 bg-white">
        ...
        <span className="font-medium text-gray-900">Sales Orders</span>
      </div>
      ...
      <div className="px-4 pt-4 pb-2">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">Manage and track all sales orders across branches</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-4">
          {stats.map((stat) => (
            <div key={stat.name} className="bg-white rounded-lg border border-gray-200 px-3 py-3 shadow-sm">
      ...
      {/* Filter / Search Bar */}
      <div className="mx-4 bg-white rounded-t-lg border border-b-0 border-gray-200">
        <div className="flex items-center gap-2 px-3 py-2.5 flex-wrap">
          ...
          <div className="relative flex-1 min-w-[200px] max-w-sm">
            ...
            <input
              ...
              className="w-full pl-8 pr-3 py-1.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-300 focus:border-primary-300"
            />
          </div>

          {/* Branch */}
          <select className="px-3 py-1.5 text-sm border border-gray-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-primary-300">
          ...
          <select
            ...
            className="px-3 py-1.5 text-sm border border-gray-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-primary-300"
          >
          ...
          <div className="ml-auto flex items-center gap-1.5">
            <button
              ...
              className={classNames(showFilters ? 'bg-primary-50 text-primary-500' : 'text-gray-500 hover:bg-gray-100', 'p-1.5 rounded-lg transition-colors')}
        ...
        {showFilters && (
          <div className="flex items-center gap-3 px-3 py-2 border-t border-gray-100 bg-gray-50/50 flex-wrap">
            <div className="flex items-center gap-2">
              <label className="text-xs text-gray-500">Customer:</label>
              <input className="px-2 py-1 text-xs border border-gray-200 rounded-md w-32 focus:outline-none focus:ring-1 focus:ring-primary-300" placeholder="Customer #" />
            </div>
            <div className="flex items-center gap-2">
              <label className="text-xs text-gray-500">Sales Rep:</label>
              <input className="px-2 py-1 text-xs border border-gray-200 rounded-md w-24 focus:outline-none focus:ring-1 focus:ring-primary-300" placeholder="0" defaultValue="0" />
            </div>
            <div className="flex items-center gap-2">
            ...
            <button className="px-3 py-1 text-xs font-medium rounded-md border border-gray-200 bg-white text-gray-600 hover:bg-gray-50">Advanced</button>
          </div>
        )}
      </div>

      {/* Data Table */}
      <div className="mx-4 flex-1 bg-white border border-gray-200 rounded-b-lg overflow-hidden flex flex-col mb-4">
        <div className="flex-1 overflow-auto">
          <table className="w-full text-xs">
            <thead className="sticky top-0 z-10">
              <tr className="bg-gray-50 border-b border-gray-200">
                <th className="px-3 py-2.5 text-left font-medium text-gray-500 uppercase tracking-wider cursor-pointer select-none hover:bg-gray-100" style={{ fontSize: '10px' }} onClick={() => toggleSort('order')}>
                  <span className="flex items-center gap-0.5">Order <SortIndicator col="order" /></span>
                </th>
                <th className="px-2 py-2.5 text-center font-medium text-gray-500 uppercase tracking-wider" style={{ fontSize: '10px', width: 40 }}>B/O</th>
                <th className="px-2 py-2.5 text-center font-medium text-gray-500 uppercase tracking-wider" style={{ fontSize: '10px', width: 40 }}>Brn</th>
                <th className="px-2 py-2.5 text-left font-medium text-gray-500 uppercase tracking-wider" style={{ fontSize: '10px', width: 80 }}>Type</th>
                <th className="px-3 py-2.5 text-left font-medium text-gray-500 uppercase tracking-wider cursor-pointer select-none hover:bg-gray-100" style={{ fontSize: '10px' }} onClick={() => toggleSort('customer')}>
                  <span className="flex items-center gap-0.5">Customer Name <SortIndicator col="customer" /></span>
                </th>
                <th className="px-2 py-2.5 text-left font-medium text-gray-500 uppercase tracking-wider cursor-pointer select-none hover:bg-gray-100" style={{ fontSize: '10px', width: 90 }} onClick={() => toggleSort('orderDate')}>
                  <span className="flex items-center gap-0.5">Order Date <SortIndicator col="orderDate" /></span>
                </th>
                <th className="px-2 py-2.5 text-left font-medium text-gray-500 uppercase tracking-wider cursor-pointer select-none hover:bg-gray-100" style={{ fontSize: '10px', width: 90 }} onClick={() => toggleSort('delivDate')}>
                  <span className="flex items-center gap-0.5">Deliv Date <SortIndicator col="delivDate" /></span>
                </th>
                <th className="px-3 py-2.5 text-left font-medium text-gray-500 uppercase tracking-wider" style={{ fontSize: '10px' }}>Delivery Address</th>
                <th className="px-2 py-2.5 text-left font-medium text-gray-500 uppercase tracking-wider" style={{ fontSize: '10px', width: 80 }}>Cust Ref</th>
                <th className="px-2 py-2.5 text-center font-medium text-gray-500 uppercase tracking-wider" style={{ fontSize: '10px', width: 65 }}>Despatch</th>
                <th className="px-2 py-2.5 text-left font-medium text-gray-500 uppercase tracking-wider cursor-pointer select-none hover:bg-gray-100" style={{ fontSize: '10px', width: 95 }} onClick={() => toggleSort('status')}>
                  <span className="flex items-center gap-0.5">Status <SortIndicator col="status" /></span>
                </th>
                <th className="px-2 py-2.5 text-left font-medium text-gray-500 uppercase tracking-wider" style={{ fontSize: '10px', width: 75 }}>Assignee</th>
                <th className="px-3 py-2.5 text-right font-medium text-gray-500 uppercase tracking-wider cursor-pointer select-none hover:bg-gray-100" style={{ fontSize: '10px', width: 110 }} onClick={() => toggleSort('totalInc')}>
                  <span className="flex items-center justify-end gap-0.5">Total INC <SortIndicator col="totalInc" /></span>
                </th>
                <th className="px-2 py-2.5 text-left font-medium text-gray-500 uppercase tracking-wider" style={{ fontSize: '10px', width: 70 }}>Cust Type</th>
            ...
            <tbody>
              {filtered.map((so, idx) => (
                <tr
                  ...
                  className="border-b border-gray-100 hover:bg-primary-50/30 transition-colors cursor-pointer group"
                  onClick={() => router.push(`/sales-orders/${so.order.replace(/^0+/, '')}-${so.bo}`)}
                >
                  <td className="px-3 py-2">
                    <button className="font-medium text-primary-500 hover:underline font-mono">{so.order}</button>
                  </td>
                  <td className="px-2 py-2 text-center text-gray-500 font-mono">{so.bo}</td>
                  <td className="px-2 py-2 text-center text-gray-500 font-mono">{so.brn}</td>
                  <td className="px-2 py-2 text-gray-700">{so.transType}</td>
                  <td className="px-3 py-2 text-gray-800 font-medium">{so.customer}</td>
                  <td className="px-2 py-2 text-gray-500 whitespace-nowrap font-mono">{so.orderDate}</td>
                  <td className="px-2 py-2 text-primary-500 whitespace-nowrap font-mono">{so.delivDate}</td>
                  <td className="px-3 py-2 text-gray-600 truncate max-w-[200px]">{so.deliveryAddress}</td>
                  <td className="px-2 py-2 text-gray-500 font-mono">{so.custRef}</td>
                  <td className="px-2 py-2 text-center text-gray-500">{so.despatch}</td>
                  <td className="px-2 py-2">
                    <span className={classNames(statusStyle(so.status), 'inline-flex px-2 py-0.5 text-xs font-medium rounded-full border')}>
                      {so.status}
                    </span>
                  </td>
                  <td className="px-2 py-2 text-gray-600">{so.assignee}</td>
                  ...
                  <td className="px-2 py-2 text-gray-500">{so.custType}</td>
                  <td className="px-2 py-2">
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={15} className="px-3 py-16 text-center text-sm text-gray-400">
        ...
        {/* Footer */}
        <div className="border-t border-gray-200 bg-gray-50 px-4 py-2.5 flex items-center justify-between text-xs shrink-0">
          <div className="flex items-center gap-4">
            <span className="text-gray-500">{filtered.length} orders</span>
            <span className="text-gray-400">|</span>
            <span className="text-gray-500">Total: <strong className="text-gray-800 font-mono">${grandTotal.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</strong></span>
          </div>
          <button className="px-3 py-1.5 text-xs font-medium rounded-md border border-gray-200 bg-white text-gray-600 hover:bg-gray-50">
            Export
          </button>
        </div>
      </div>
